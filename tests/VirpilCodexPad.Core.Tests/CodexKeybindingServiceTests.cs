...
public sealed class CodexKeybindingServiceTests
{
    ...
    public async Task ProvisioningPreservesEntriesAndRunsOnlyOnce()
    {
        ...
        Assert.Contains(
            ...
            path => path.Contains("keybindings.json.virpil-codex-pad-", StringComparison.Ordinal));
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "VirpilCodexPadTests", Guid.NewGuid().ToString("N"));
