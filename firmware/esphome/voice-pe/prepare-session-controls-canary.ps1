...
$basePreparationScript = Join-Path $PSScriptRoot "prepare-tunable-wake-canary.ps1"
$overlayPath = Join-Path $PSScriptRoot "joydex-voice-pe-session-controls-canary.yaml"
$patchPaths = @(
...
if ($LASTEXITCODE -ne 0) {
    throw "The reviewed 0.1.34 source preparation failed."
}
...
$preparedScoreSoundPath = Join-Path $destinationPath "joydex-voice-pe-score-sound-canary.yaml"
...
$preparedAudioPath = Join-Path $destinationPath "joydex-voice-pe-audio-canary.yaml"
...
$preparedOverlayPath = Join-Path $destinationPath "joydex-voice-pe-session-controls-canary.yaml"
...
Write-Output "Prepared source-only Voice PE 0.1.41 session-controls-and-cues candidate at $destinationPath"
Write-Output "Reviewed 0.1.34 base, center-button controls, and one-shot ready/end cues: PASS"
