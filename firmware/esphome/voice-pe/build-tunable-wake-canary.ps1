...
param(
    ...
    [string]$StageRoot = (Join-Path $PSScriptRoot "..\..\..\.tools\voice-pe-tunable-wake-0.1.34-staged"),
...
$preparationScript = Join-Path $PSScriptRoot "prepare-tunable-wake-canary.ps1"
...
$configurationName = "joydex-voice-pe-tunable-wake-canary.yaml"
$sourceDateEpoch = "1787961600"

$expectedEspHomeVersion = "2025.12.2"
$expectedFirmwareVersion = "0.1.34"
...
$expectedOverlaySha256 = "C5986C8DCA8D26783769A352977809D926A7DE960012B7BCBC08FC4E7E20FE34"
$expectedDependencyLockSha256 = "C725F1C549D0BD73E84F5B9CDD89272E1CD53930F89E05EC9620A8264D96771B"
$baselineTotalImageBytes = 3149203
$baselineStaticRamBytes = 46736
...
$expectedInputClosureSha256 = "83A2F459D3673BCF6725B822DF65623F47B91F416DC7ABF4A15C8066BE30BAE1"
...
function Assert-GeneratedCandidate([string]$Phase) {
    ...
    foreach ($evidence in @(
        ...
        @{ Path = $generatedMainPath; Pattern = 'okay_computer = new micro_wake_word::WakeWordModel("okay_computer",' },
        @{ Path = $generatedMainPath; Pattern = ', 89, 5, "Okay Computer", 30000, true, false);' },
        @{ Path = $generatedMainPath; Pattern = 'App.register_number(joydex_wake_probability_cutoff);' },
        @{ Path = $generatedMainPath; Pattern = 'joydex_wake_probability_cutoff->set_initial_value(0.35f);' },
        @{ Path = $generatedMainPath; Pattern = 'joydex_wake_probability_cutoff->set_restore_value(true);' },
        @{ Path = $generatedMainPath; Pattern = 'okay_computer->set_probability_cutoff(cutoff);' },
...
$manifest = [ordered]@{
    ...
    change = "Expose one persisted wake probability cutoff while retaining the reviewed 0.1.33 audio and inference baseline"
...
Write-Output "Verified Voice PE $expectedFirmwareVersion tunable-wake candidate build complete. No device connection or upload was attempted."
