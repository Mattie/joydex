...
foreach ($requiredPath in @(
    ...
    if (-not (Test-Path -LiteralPath $requiredPath)) {
        throw "A required modern Sendspin candidate source is missing: $requiredPath"
    }
}

& $basePreparationScript -Destination $destinationPath -SecretsPath $SecretsPath
if ($LASTEXITCODE -ne 0) {
    throw "The exact 0.1.26 Voice PE base preparation failed."
}
...
$preparedOverlayPath = Join-Path $destinationPath "joydex-voice-pe-modern-sendspin-canary.yaml"
...
Write-Output "Prepared source-only Voice PE 0.1.33 modern Sendspin watchdog-fix candidate at $destinationPath"
Write-Output "Pinned media/speaker ABI, atomic playback backports, adapter source, and candidate overlay parity: PASS"
