...
$overlayPath = Join-Path $PSScriptRoot "joydex-voice-pe-audio-canary.yaml"
...
$expectedOverlaySha256 = "CF85BB974DDDAF0878305E4A60CB5D7CF8B56A2EEC016517929AFC21BC0B7CAD"
...
if ((Get-FileHash -LiteralPath $overlayPath -Algorithm SHA256).Hash -ne $expectedOverlaySha256) {
    throw "The Voice PE audio overlay differs from the reviewed 0.1.26 async-uplink configuration."
}
...
Copy-Item -LiteralPath $overlayPath -Destination (Join-Path $destinationPath "joydex-voice-pe-audio-canary.yaml") -Force
...
$preparedOverlayHash = (Get-FileHash -LiteralPath (Join-Path $destinationPath "joydex-voice-pe-audio-canary.yaml") -Algorithm SHA256).Hash
if ($sourceOverlayHash -ne $preparedOverlayHash) {
    throw "The prepared 0.1.26 overlay differs from tracked source."
}
...
Write-Output "Prepared source-only Voice PE 0.1.26 async-uplink LAN bridge at $destinationPath"
Write-Output "LAN component, stock-Sendspin overlay, and ready/end-cue source parity: PASS"
